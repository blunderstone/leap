# Best Practices: Managing Claude Sessions with LEAP
...
This document provides best practices for managing Claude Code session boundaries when working with LEAP (Literate Extended-by-Agent Programming) feature branches. Effective session management reduces token overhead, maintains focus, and leverages LEAP's self-documenting nature.
...
When a Claude Code conversation approaches the token limit (~200k tokens), the system automatically **compacts** (summarizes) the conversation:

- Preserves full conversation history in a condensed summary
- Resets token budget to zero (fresh 200k tokens)
- Allows seamless continuation without manual intervention
- Summary includes implementation details, discussions, iterations from all phases

### Why Clean Sessions Are Better

Starting a **new session** at logical breakpoints is preferable because:

1. **LEAP docs already capture state**: goals.md + plan.md + success criteria + phase artifacts = complete picture
2. **Compaction preserves noise**: Implementation details from completed phases aren't needed going forward
3. **Lower token overhead**: Reading 3-4 LEAP docs (~5-10k tokens) vs compacted summary (~15k+ tokens with irrelevant details)
4. **Focused context**: Only load what's relevant to the next phase
5. **Clear mental model**: Each session maps to logical work units

### Example: Why Details Become Noise

#### Phase 1 Complete (ADR creation)

- Compaction preserves: Draft iterations, formatting discussions, ADR structure debates
...
- Compaction preserves: Function signature discussions, KDoc iteration, commit message crafting
- Clean session needs: Read goals.md showing Phase 3 [x] complete, understand changes were made
...
**Exception:** Multiple simple phases that won't trigger compaction can share a session.
...
Read the essential LEAP documents to understand current state:
...
- Update plan.md with "Decision Points" section if approaches change
...
- Read goals.md, plan.md, audit findings
...
- **Decision point**: Even though Phase 3 could fit, start new session to avoid compaction mid-phase
...
3. **Better focus**: Only load context relevant to current phase
